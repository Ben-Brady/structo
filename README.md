...
class WavHeader(st.Struct):
    ...
    file_size: t.Annotated[int, st.u32_LE]
    format: t.Annotated[str, st.Literal(b"WAVE")]


class ChunkHeader(st.Struct):
    id: t.Annotated[str, st.Buffer(4)]
    size: t.Annotated[int, st.u32_LE]


class WavInfoChunk(st.Struct):
    audio_format: t.Annotated[int, st.u16_LE]
    num_channels: t.Annotated[int, st.u16_LE]
    sample_rate: t.Annotated[int, st.u32_LE]
    byte_range: t.Annotated[int, st.u32_LE]
    block_align: t.Annotated[int, st.u16_LE]
    bits_per_sample: t.Annotated[int, st.u16_LE]
